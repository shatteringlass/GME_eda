...
function getSID (){
  SID="$(grep -i asp cookie.txt | cut -f7)"
  echo "$SID"
}
...
function getDate (){
  # Base case is BSD/macOS
  BSD_OPT="-j -f %Y%m%d"
  if [[ "$(uname)" -eq "Linux" ]]; then BSD_OPT="-d"; fi
  args=""$BSD_OPT" "$1" "+%s""
...
function toYmd (){
  ...
  BSD_OPT="-j -f %s "
  if [[ "$(uname)" -eq "Linux" ]]; then BSD_OPT="-d @"; fi
  echo "$(date $BSD_OPT$1 "+%Y%m%d")"
}

# Takes startDate and endDate in YYYYMMDD format as input, converts to epoch-based and downloads corresponding XML MGP files
echo -e "Going to fetch MGP outcomes for dates between $1 and $2\n"
